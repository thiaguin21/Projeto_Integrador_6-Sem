...
function carregarTabela() {
    ...
    produtos.forEach((produto, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button onclick="confirmarAcao('editar', ${index})">Editar</button>
                <button onclick="confirmarAcao('excluir', ${index})">Excluir</button>
            </td>
        `;
...
function editarProduto(index) {
    ...
    // Salva o produto em edição no Local Storage
...
excluirSelecionadosBtn.addEventListener("click", () => {
    ...
    if (confirm(`Tem certeza que deseja excluir ${selecionados.length} produto(s)?`)) {
        ...
        // Exclui os produtos em ordem inversa para evitar problemas de reindexação
